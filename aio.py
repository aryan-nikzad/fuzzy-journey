...
def main() -> None:
    ...
    # -----------------------------------------------------------
    # MODE 1: show-only (never touches or downloads data)
    # -----------------------------------------------------------
    if args.today or args.file or (args.date and not args.override):
        pass  # fall through to the show path below when no download happens

    # -----------------------------------------------------------
    # SHOW PATH (existing data, no download needed)
    # -----------------------------------------------------------
    def show_schedule(explicit_path: Optional[Path] = None, show_today: bool = True) -> int:
        ...
        if args.date:
            try:
                ...
                print(f"--date must be in DD-MM-YYYY format, got: {args.date}")
                return 1
        ...
        if show_today:
            ...
            print(f"Date                  : {target_date.strftime(CSV_DATE_FORMAT)} ({label})")
    ...
    # -----------------------------------------------------------
    if args.file:
        return show_schedule(Path(args.file), show_today=True)

    # -----------------------------------------------------------
    # SHOW-TODAY (or --date) with no download requested.
    # -----------------------------------------------------------
    if args.today or (args.date and not args.override):
        return show_schedule(show_today=True)
    ...
    # Warn if the existing schedule doesn't cover today (stale data).
    if do_download is False:
        rows = load_rows(SCHEDULE_FILE)
        if find_row_for_date(rows, date.today()) is None:
            print("Note: existing schedule does not contain today's date —")
